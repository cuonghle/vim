...
augroup endwise " {{{1
  ...
  autocmd FileType verilog
        ...
        \ let b:endwise_words = 'begin,module,function,task,program,class' |
        \ let b:endwise_syngroups = 'verilogLabel,verilogStatement'
  autocmd FileType systemverilog
        \ let b:endwise_addition = '\=submatch(0)=="begin" ? "end" : "end".submatch(0)' |
        \ let b:endwise_words = 'begin,module,function,task,program,class' |
        \ let b:endwise_syngroups = 'systemverilogKeyword,verilogLabel,verilogStatement'
augroup END " }}}1
